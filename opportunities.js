...
const SYSTEM = 'You are SydCrest Launchpad\'s opportunity intelligence assistant for tech talent in Ghana and West Africa. Be practical, accurate, and Africa-focused. Always consider that the user is applying from Ghana.';

router.get('/', auth, async (req, res) => {
  const { data } = await supabase.from('opportunities').select('*').eq('user_id', req.user.id).order('created_at', { ascending: false });
...
router.post('/:id/research', auth, async (req, res) => {
  ...
  try {
    const response = await client.messages.create({
      ...
      messages: [{ role: 'user', content: `Research this opportunity for a tech talent applying from Ghana.
...
router.post('/:id/roadmap', auth, async (req, res) => {
  ...
  try {
    const response = await client.messages.create({
      ...
      messages: [{ role: 'user', content: `Create a backwards-planned application roadmap. Applicant is from Ghana.
...
router.post('/:id/assistant', auth, async (req, res) => {
  ...
  try {
    const stream = await client.messages.stream({
      ...
      system: `You are a personal career assistant for ${req.user.full_name}, a tech talent from Ghana. Opportunity: ${opp.type} — ${opp.title} at ${opp.org}. Deadline: ${opp.deadline}. DO the hard work: draft full emails, write complete cover letters, build checklists. Produce ready-to-use content.`,
